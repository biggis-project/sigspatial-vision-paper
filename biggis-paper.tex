...
We group related work into geo-spatial big data-analytics platforms and uncertainty handling. The former covers works on big data enabled GIS platforms and discusses their conceptual designs. The latter gives an overview of fields of research with respect to uncertainty handling and knowledge generation.

\subsection{Geo-Spatial Big Data-Analytics Platforms}
\begin{itemize}
	\item \textcolor{red}{ToDO@Patrick, maybe better to leave out completely or put into Section \ref{sec:chls}} Challenges related to the nature of big data, i.e. large, fast and diverse, lead to the evolution of new big data management and analytics architectures. An interesting architecture proposal is the lambda architecture \cite{Marz2013}. By decomposing the problem into three layers, namely the batch layer, the speed layer, and the serving layer this architecture hybridly combines historically known batch analytics and newly arising stream analytics to overcome eachs single weakenesses. While this design allows for adhoc analysis ability during batch runs the processing logic has to be implemented twice, in the batch and speed layer, which increases the overall complexity. A more recent approach known as the kappa architecture \cite{Kreps2014} tries to decrease the introduced complexity on analytics level by exclusively using speed and serving layer. 
...
%\end{itemize}
In this section, we briefly describe the continuous refinement model in BigGIS, consisting of an integrated analytics pipeline which blends analytics and semantic reasoning on system-side with knowledge extraction and generation on human-side and thereby modelling uncertainty to continuously refining results as shown in Figure \ref{fig:biggisworkflow}.

%Providing meaningful predictive and prescriptive decision support is no trivial task. Spatially distributed data sources volunteered geographic information and participatory sensing data Fortunately, semantics reasoning and human-related knowledge extraction and generation help to 
...
When considering uncertain data like volunteered geographic information and participatory sensing data in geographic information systems, we also have to deal with uncertainty. To address this challenge, we apply semantic reasoning on the provenance information of data sources in order to infer a level of uncertainty that can be considered in the analytical processes.
