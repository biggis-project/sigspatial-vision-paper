...
\begin{itemize}
	...
	\item \textcolor{red}{ToDO@Patrick} Thakur et al. introduces PlanetSense, a real-time streaming and spatio-temporal analytics platform for gathering geo-spatial intelligence from open source data \cite{Thakur2015}. Based on the lambda architecture design, this approach has ...
...
%\end{itemize}
 We consider uncertainty to be reciprocally related to generating new insights and consequently knowledge. Thus modelling uncertainty in BigGIS is a crucial task. In this section, we briefly describe the continuous refinement model in BigGIS, consisting of an integrated analytics pipeline which blends analytics and semantic reasoning on system-side with knowledge extraction and generation on human-side and thereby modelling uncertainty to continuously refine results as shown in Figure \ref{fig:biggisworkflow}. 
...
The analytics pipeline is the core of the continuous refinement model. A key abstraction within this model are specific access points called \textit{refinement gates} (see yellow squares in Figure \ref{fig:biggisworkflow}). Refinement gates allow for semantics and external expert knowledge to enter the pipeline at arbitrary stages during analyses to continuously improve data management and analytics results, e.g. to support data preparation and autonomously deployed data transformations, to provide domain knowledge to train machine learning models for detecting patterns in remotely sensed data or to manipulate visualizations. We think of this as a knowledge transfer from human to machine. Thus, providing indirect support for the users findings during the course of exploration.
...
\textcolor{red}{ToDO@Matthias: direct and indirect semantic support}

\subsubsection{Knowledge Extraction and Generation}
\textcolor{red}{ToDO@Manuel@Daniel: direct and indirect expert support}
...
\textcolor{red}{ToDO@Manuel@Daniel: VA-level}\newline
\textcolor{red}{ToDO@Julian: analytics-level}
