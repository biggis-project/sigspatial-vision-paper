...
\\
\textcolor{red}{ToDO@Julian} (Analytics aka Magic)
\\
...
\subsubsection{Knowledge Extraction and Generation}
 The user the is another dominating part in the continuous refinement model that is either provided with additional expert knowledge by another physical person (direct expert knowledge) or himself is the expert in a specific field of application. Overall, we see the continuous refinement process as a knowledge transfer from human (expert knowledge) to system which is reinforced by semantic reasoning. Thereby, human knowledge is introduced to the system that can contain additional domain specific information and constraints. By doing so, big data analytics can 
\begin{inparaenum}[(1)]
...
\begin{inparaenum}[(1)]
	\item smart city and health, i.e. urban heat islands problem,
	\item environmental management, i.e. spread of invasive pests,
	\item emergency management, i.e. dispersion of toxic gases.
\end{inparaenum}
